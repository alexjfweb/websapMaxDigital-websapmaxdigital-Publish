...
export default function CartSummary({ cart, restaurantPhone }: CartSummaryProps) {
  ...
  const handleSendOrderViaWhatsApp = () => {
    ...
    let message = `Hello ${mockRestaurantProfile.name}! I'd like to place the following order:\n\n`;
    ...
    toast({
      ...
      description: "Your order is ready to be sent via WhatsApp.",
    });
  };
  ...
  return (
    <Card className="shadow-xl">
      <CardHeader>
        ...
        <CardDescription>Review items and proceed to order.</CardDescription>
      </CardHeader>
      <CardContent>
        {cart.items.length === 0 ? (
      ...
      <CardFooter className="flex flex-col gap-3">
        ...
        >
          <WhatsAppIcon className="mr-2 h-5 w-5" /> Send Order via WhatsApp
        </Button>
        {cart.items.length > 0 && (

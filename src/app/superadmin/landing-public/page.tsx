  ...
  const handleSubsectionImageUpload = async (e: ChangeEvent<HTMLInputElement>, sectionIndex: number, subIndex: number) => {
    ...
    try {
      ...
      if (imageUrl) {
        ...
        // Esto se ejecutará si la función de subida devuelve null, como en el caso de error sugerido.
      ...
      // El try/catch en el servicio debería manejar esto, pero es bueno tener una segunda barrera.
        ...
        <div class="space-y-6">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="hero" className="space-y-4">
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Type class="w-5 h-5" />Configuración del Hero</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div class="grid grid-cols-3 gap-4">
                    <div><Label htmlFor="heroBackgroundColor">Color de Fondo</Label><div class="flex gap-2"><Input id="heroBackgroundColor" type="color" value={formData.heroBackgroundColor} onChange={(e) => setFormData(prev => prev ? ({ ...prev, heroBackgroundColor: e.target.value }) : null)} className="w-16 h-10"/><Input value={formData.heroBackgroundColor} onChange={(e) => setFormData(prev => prev ? ({ ...prev, heroBackgroundColor: e.target.value }) : null)} placeholder="#ffffff"/></div></div>
            ...
            <TabsContent value="seo" className="space-y-4">
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Globe class="w-5 h-5" />Configuración SEO</CardTitle></CardHeader>
                <CardContent className="space-y-4">
              ...
              <Card><CardHeader><CardTitle>Vista Previa de la Landing</CardTitle></CardHeader>
                <CardContent><div class="border rounded-lg p-4 bg-gray-50"><div class="text-center py-12" style={{ backgroundColor: formData.heroBackgroundColor, color: formData.heroTextColor}}><h1 class="text-4xl font-bold mb-4">{formData.heroTitle}</h1><p class="text-xl mb-8">{formData.heroSubtitle}</p><button className="px-8 py-3 rounded-lg font-semibold" style={{ backgroundColor: formData.heroButtonColor, color: '#ffffff' }}>{formData.heroButtonText}</button></div>
                    {formData.sections.filter(s => s.isActive).map((section) => (<div key={section.id} className="py-12 px-4" style={{ backgroundColor: section.backgroundColor, color: section.textColor}}><div class="max-w-4xl mx-auto text-center"><h2 class="text-3xl font-bold mb-4">{section.title}</h2>{section.subtitle && (<p class="text-xl mb-6">{section.subtitle}</p>)}<p class="mb-8">{section.content}</p><button className="px-6 py-2 rounded-lg font-semibold" style={{ backgroundColor: section.buttonColor, color: '#ffffff' }}>{section.buttonText}</button></div>
                        ...
                        {(section.subsections || []).length > 0 && (<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">{section.subsections.map(sub => (<div key={sub.id} className="bg-white rounded-lg shadow p-4 flex flex-col items-center">{sub.imageUrl && <img src={sub.imageUrl} alt="sub" className="mb-2 w-20 h-20 object-cover rounded-full" />}<h4 class="font-bold text-lg mb-1">{sub.title}</h4><p class="text-sm text-center">{sub.content}</p></div>))}</div>)}
                      </div>))}

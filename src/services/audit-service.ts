...
export interface AuditLog {
  id: string;
  entity: 'landingPlans' | 'companies' | 'users' | 'tables' | 'reservations' | 'orders' | 'landingConfigs';
  entityId: string;
  action: 'created' | 'updated' | 'deleted' | 'reordered' | 'status_changed' | 'role_changed' | 'rollback';
  performedBy: {
    uid: string;
    email: string;
  };
  timestamp: Date; // Usar objeto Date para facilitar el manejo
  previousData?: any;
  newData?: any;
  ...
  async getLogs(options: { entity?: string; action?: string; limit?: number } = {}): Promise<AuditLog[]> {
    ...
    return snapshot.docs.map(doc => {
        ...
        return { 
            id: doc.id,
            ...data,
            timestamp: (data.timestamp as Timestamp).toDate(), // Convertir a objeto Date
        } as AuditLog;
    });
  }
}
